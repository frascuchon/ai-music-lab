-- AI Music Lab - Setup panel (global configuration wizard)
...
--  MidiGenerator: prewarm per-model weights).
...
local function plugin_exists(name)
  ...
  test = io.open(PLUGINS_DIR .. name .. "/panel.lua", "r")
  if test then test:close(); return true end
  return false
end
...
local PF = {
  ...
  abcmidi      = TMPDIR .. "reaperai_setup_abcmidi.txt",
  ...
  prw_mg_cm_adapter   = TMPDIR .. "reaperai_setup_prw_mg_cm_adapter.txt",
...
local function init_checks()
  ...
  local labels = {
    ...
    abcmidi       = "abcmidi / midi2abc  (MidiGenerator ChatMusician seed)",
  }
...
local ST = {
  ...
  abcmidi_state   = "idle", abcmidi_msg   = "",
  ...
  prw_mg_cm_adapter_state   = "idle", prw_mg_cm_adapter_msg   = "", prw_mg_cm_adapter_pct   = 0.0,
...
function M.poll()
  ...
  poll_simple("abcmidi_state", "abcmidi_msg", "abcmidi")
  ...
  if HAS_MG then
    ...
    poll_prewarm("prw_mg_cm_adapter_state", "prw_mg_cm_adapter_msg", "prw_mg_cm_adapter_pct", "prw_mg_cm_adapter")
  ...
  maybe_recheck("abcmidi_state")
  if pending_recheck and ST.check_state ~= "running" then
    pending_recheck = false; run_check()
  end
end

-- Returns a list of missing core check labels — used by the container
-- for the "incomplete setup" banner shown on the other tabs.
function M.get_missing()
  ...
  if ST.check_state ~= "done" then return missing end
  for _, c in ipairs(CHECKS) do
    if LABELS[c.name] and c.status == "missing" then
      table.insert(missing, LABELS[c.name])
    end
  end
  return missing
end
...
function M.draw()
  ...
  if HAS_MG then
    ...
    -- abcmidi (midi2abc/abc2midi) — required locally for ChatMusician's
    -- "harmonize/simplify with seed" flow (MIDI<->ABC conversion happens on
    -- this machine, before the Modal call, not inside the container).
    do
      local c = CHECKS[CHECK_IDX["abcmidi"]]
      if c then
        local icon  = checking and "..." or status_icon(c.status)
        local color = checking and "YELLOW" or status_color(c.status)
        g.text_colored("[" .. icon .. "]", color)
        ...
        if not checking and c.status == "missing" then
          g.same_line()
          g.begin_disabled(ST.abcmidi_state == "running")
          if g.button("Install", t.sc(70), t.ITEM_H) then
            ST.abcmidi_state = "running"; ST.abcmidi_msg = ""
            launch("abcmidi", "install-abcmidi")
          end
          g.end_disabled()
          if ST.abcmidi_state == "running" then
            g.same_line(6); g.text_colored("Installing...", "YELLOW")
          elseif ST.abcmidi_state == "error" then
            g.same_line(6)
            g.text_colored(ST.abcmidi_msg ~= "" and ST.abcmidi_msg:sub(1,55) or "Error — check the log", "RED")
          end
        end
      end
    end
    ...
    g.text_colored("[OK]", "GREEN")
    g.same_line(6)
    g.text("No other local dependencies — weights in Modal Volumes")
    g.spacing()
    ...
    local MG_PREWARM = {
      ...
      { key="cm_adapter",   label="Download ChatMusician prompt adapter (~3 GB)", cost="T4", state="prw_mg_cm_adapter_state", msg="prw_mg_cm_adapter_msg", pct="prw_mg_cm_adapter_pct", subcmd="prewarm-midigen-chatmusician-adapter" },
